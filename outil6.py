...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    ...
    # Onglet 3: Durée entre versions de documents
        ...
        # Choisir le type de calcul (moyenne ou maximum) et la catégorie (TYPE DE DOCUMENT)
        type_calcul = st.selectbox('Sélectionnez le type de calcul', ['mean', 'max'], key='calcul_duree_versions_type')
        
        # Calculer les résultats en fonction de la sélection de l'utilisateur
        if type_calcul == 'mean':
            resultats = donnees.groupby('TYPE DE DOCUMENT')['Durée entre versions'].mean().reset_index()
            title = f'Durée moyenne entre versions (jours) par TYPE DE DOCUMENT'
        elif type_calcul == 'max':
            resultats = donnees.groupby('TYPE DE DOCUMENT')['Durée entre versions'].max().reset_index()
            title = f'Durée maximum entre versions (jours) par TYPE DE DOCUMENT'
        ...
        # Ajouter la ligne de moyenne globale pour les durées moyennes
        if type_calcul == 'mean':
            moyenne_globale = resultats[resultats.columns[1]].mean()
    ...
    # Onglet 8: Calendrier des Projets 
        ...
        donnees_gantt['Date fin'] = donnees_gantt.apply(lambda x: x['Date fin'] si x['Durée en jours'] > 0 else x['Date début'] + pd.Timedelta(days=1), axis=1)
