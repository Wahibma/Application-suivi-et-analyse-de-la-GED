...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    ...
    if selectionne == "Répartition des Catégories de documents":
        ...
        moyenne_indices_par_type = donnees.groupby('Catégories de documents')['Nombre d\'indices'].mean().reset_index()
        moyenne_indices_par_type.columns = ['Catégories de documents', 'Nombre moyen d\'indices']
        fig = px.pie(moyenne_indices_par_type, values='Nombre moyen d\'indices', names='Catégories de documents', title='Répartition des Catégories de documents', hole=0.5)  # Ajouter un trou au centre
