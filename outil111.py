...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    ...
    # Onglet 3: Durée entre versions de documents
        ...
        # Calculer la durée entre versions pour chaque document
        donnees = donnees.sort_values(by=['Libellé du document', 'Date dépôt GED'])
        donnees['Durée entre versions'] = donnees.groupby('Libellé du document')['Date dépôt GED'].diff().dt.days
        donnees['Durée entre versions'] = donnees['Durée entre versions'].fillna(0)

        # Vérifier si l'utilisateur a sélectionné 'mean' ou 'max'
        ...
        if type_calcul == 'mean':
            # Calculer la durée moyenne entre versions par type de document ou lot
            ...
            # Calculer la durée maximale entre versions par type de document ou lot
        ...
        # Trier les résultats par durée
        resultats = resultats.sort_values(by=resultats.columns[1], ascending=False)

        # Générer le graphique en barres
        fig = px.bar(resultats, x=categorie, y=resultats.columns[1], title=title, color=categorie, color_discrete_sequence=generate_dynamic_colors(len(resultats[categorie])))
        fig.add_hline(y=resultats[resultats.columns[1]].mean(), line_dash="dash", line_color="red", annotation_text=f"Moyenne: {resultats[resultats.columns[1]].mean():.2f} jours")
