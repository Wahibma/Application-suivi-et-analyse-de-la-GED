...
def generate_unique_colors(categories):
    ...
    for i, category in enumerate(categories):
        color_map[category] = unique_colors[i % len(unique_colors)]
    return color_map
...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    ...
    # Onglet 3: Analyse des documents par lot et indice
        ...
        color_map_lot = generate_unique_colors(documents_par_lot['LOT'])
        documents_par_lot['Couleur'] = documents_par_lot['LOT'].map(color_map_lot)
        ...
        fig_bar_lot = px.bar(
            ...
            color='Couleur',
            color_discrete_sequence=documents_par_lot['Couleur']
        )
        ...
        color_map_type = generate_unique_colors(documents_par_type['TYPE DE DOCUMENT'])
        documents_par_type['Couleur'] = documents_par_type['TYPE DE DOCUMENT'].map(color_map_type)
        ...
        fig_bar_type = px.bar(
            ...
            color='Couleur',
            color_discrete_sequence=documents_par_type['Couleur']
        )
    ...
    # Onglet 5: Analyse de la masse de documents par projet
        ...
        def mise_a_jour_analyse_masse_documents(projets_selectionnes, periode_selectionnee):
            ...
            color_map_chantier = generate_unique_colors(df_barre['Chantier'])
            df_barre['Couleur'] = df_barre['Chantier'].map(color_map_chantier)
    ...
    # Onglet 6: Nombre d'indices par type de document
        ...
        if representation == "Tableau":
            ...
            # Générer des couleurs uniques pour chaque type de document
            color_map_type = generate_unique_colors(resultats['TYPE DE DOCUMENT'])
            resultats['Couleur'] = resultats['TYPE DE DOCUMENT'].map(color_map_type)

            fig = px.bar(resultats, x='TYPE DE DOCUMENT', y=resultats.columns[1], title=title, color='Couleur', color_discrete_sequence=resultats['Couleur'])
    ...
    # Onglet 7: Durée entre versions de documents
        ...
        if representation == "Tableau":
            ...
            color_map_categorie = generate_unique_colors(resultats[categorie])
            resultats['Couleur'] = resultats[categorie].map(color_map_categorie)

            fig = px.bar(resultats, x=categorie, y=resultats.columns[1], title=title, color='Couleur', color_discrete_sequence=resultats['Couleur'])
