...
# Fonction pour calculer les statistiques descriptives et corrélations
def calculer_statistiques_et_correls(donnees):
    st.header("Statistiques Descriptives et Corrélations")
    ...
    # Ajout du calcul de l'écart-type
    ecart_type_documents = stats_nombre_documents.std()
    ecart_type_indices = moyennes_indices.std()
    ecart_type_duree = duree_moyenne_versions.std()

    # Création du tableau des statistiques avec l'écart-type
    tableau_statistiques = pd.DataFrame({
        'Statistique': ['Nombre de documents', 'Moyenne des indices', 'Durée entre versions'],
        ...
        'Écart-type': [ecart_type_documents, ecart_type_indices, ecart_type_duree]
    })
    ...
    stats_corr = pd.DataFrame({
        'Nombre de documents': stats_nombre_documents,
        'Moyenne des indices': moyennes_indices,
        'Durée entre versions': duree_moyenne_versions
    })
...
def afficher_graphique(selectionne, donnees, projets, projet_selectionne):
    if selectionne == "Analyse Exploratoire":
        calculer_statistiques_et_correls(donnees)
